...
if (db.usage){
  ...
  # fixed part
  #ex.dim=data.table(dbGetQuery(conn,paste("select * from opt_modelinput_dim where client_id=",client_id,sep="")))
  ...
  ####################################################################################
  # customized part
  ex.media=fread("opt_input_dim_chan.csv",drop="client_id",na.strings="NULL")
  ...
  tax=0 # tax rate for ROI
  max.level=1e+10 # max constrain if missing or goal seek 
  beta="a" # para name of beta in curve formula
  path="d:\\Archives\\Git\\opt\\" # source code path
